...
# helper function to find largest element in BST - it is the rightmost element
...
def find_second_largest(root):
    if not root or (not root.left and not root.right):
        raise ValueError('Tree must have atleast 2 nodes')

    # Find the second largest item in the binary search tree
    while root:
        # case where the largest has left subtree
        if root.left and not root.right:
            return find_largest(root.left)

        
        # case where the largest has no left subtree -> second largest is the parent
